...
function isBusinessHours(): boolean {
  ...
  return est.getHours() >= 8 && est.getHours() < 18 && isBusinessDay(est);
}
...
export default function Dashboard() {
  ...
  const [live, setLive] = useState(isBusinessHours());

  useEffect(() => {
    supabase.from("rh_clients").select("*").order("name").then(({ data }) => { if (data) setClients(data); });
  }, []);

  useEffect(() => {
    const iv = setInterval(() => { setEstTime(getESTTime()); setLive(isBusinessHours()); }, 1000);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const q = (date: string) => {
        let query = supabase.from("rh_daily_logs").select("*").eq("date", date);
        if (selectedClient !== "all") query = query.eq("client_id", selectedClient);
        return query;
      };

      const [{ data: todayLogs }, { data: yesterdayLogs }] = await Promise.all([q(today), q(yesterday)]);

      const calcStats = (logs: any[] | null) => ({
        calls: (logs ?? []).reduce((s: number, l: any) => s + (l.calls_made || 0), 0),
        collections: (logs ?? []).filter((l: any) => l.service_type === "payment").reduce((s: number, l: any) => s + (l.amount_collected || 0), 0),
        confirmations: (logs ?? []).filter((l: any) => l.service_type === "appointment" && l.outcome === "confirmed").length,
        ccm: (logs ?? []).filter((l: any) => l.service_type === "ccm" && l.outcome === "checked").length,
      });

      setStats(calcStats(todayLogs));
      setPrevStats(calcStats(yesterdayLogs));
      ...
      if (weekLogs) {
        ...
        setWeeklyData(Object.entries(byDate).map(([date, v]) => ({
          date: new Date(date).toLocaleDateString("en-US", { weekday: "short", day: "numeric" }), ...v,
        })));
      }
  ...
  useEffect(() => {
    fetchData().then(() => { setTimeout(() => { if (feed.length === 0) fetchData(); }, 5000); });
  }, [fetchData]);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      {holiday && (
        <div className="mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <span className="text-amber-500 text-xs ml-2">({holiday})</span>
        </div>
      )}
      {weekend && !holiday && (
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-0.5">
            <span className="text-xs text-slate-400">
              ...
              <button onClick={fetchData} className="ml-2 text-teal-500 hover:text-teal-600 inline-flex items-center gap-1"><RefreshCw size={10} /></button>
            </span>
            <div className="flex items-center gap-1.5 text-xs">
              <Clock size={10} className="text-slate-400" />
              <span className="font-mono text-slate-500">{estTime} EST</span>
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-semibold ${live ? "bg-emerald-50 text-emerald-600" : "bg-slate-100 text-slate-400"}`}>
                {live ? "LIVE" : "AFTER HOURS"}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon={<Phone size={20} />} label="Total Calls Today" value={stats.calls} change={pct(stats.calls, prevStats.calls)} />
        <StatCard icon={<DollarSign size={20} />} label="Collections Today" value={stats.collections} prefix="$" change={pct(stats.collections, prevStats.collections)} />
        <StatCard icon={<CalendarCheck size={20} />} label="Appointments Confirmed" value={stats.confirmations} change={pct(stats.confirmations, prevStats.confirmations)} />
        <StatCard icon={<HeartPulse size={20} />} label="CCM Check-ins" value={stats.ccm} change={pct(stats.ccm, prevStats.ccm)} />
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
          ...
          <div className="space-y-2.5">
            {leaderboard.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-4">No activity today</p>
            ) : leaderboard.map((a, i) => (
              <div key={i} className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <span className="text-sm font-semibold text-slate-900">{a.calls}</span>
                  {a.collected > 0 && <span className="text-[10px] text-emerald-600 ml-2">${a.collected.toLocaleString()}</span>}
                </div>
              </div>
            ))}
          </div>
